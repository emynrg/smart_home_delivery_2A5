...
bool historique::ajouter()
{
  ...
  //QString cin_string= QString::number(cin);
         query.prepare("INSERT INTO historique_clients (id_historique, nom_historique, prenom_historique,cin_historique,num_telephone_historique,date_commande_historique,promotion_historique) "
...
{
    QSqlQuery query;
         query.prepare("Delete from historique_clients where id_historique=:id");
...
bool historique::modifier(QString id,QString nom, QString prenom,QString cin, QString num, QDate date, QString promotion )
{
    ...
    query.prepare("update historique_clients set nom_historique=:nom,prenom_historique=:prenom,cin_historique=:cin,num_telephone_historique=:num ,date_commande_historique=:date ,promotion_historique=:promotion where id_historique=:id");
    query.bindValue(":id", id);
    query.bindValue(":nom",nom);
    query.bindValue(":prenom",prenom);
    query.bindValue(":cin", cin);
...

{
    ...
    model->setQuery("select * from historique_clients where ((id_historique|| nom_historique|| prenom_historique||cin_historique||num_telephone_historique||date_commande_historique||promotion_historique) LIKE '%"+aux+"%')");
...
QSqlQueryModel *  historique::trier(const QString &critere, const QString &mode )
{
    QSqlQueryModel * model= new QSqlQueryModel();

model->setQuery("select * from historique_clients order by "+critere+" "+mode+"");
